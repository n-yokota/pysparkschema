...
def merge_array_schemas(a1, a2):
    ...
    if a1_elem_typename == a2_elem_typename:
        ...
        return ArrayType(a1.elementType)
    print(a1_elem_typename, a2_elem_typename)

    raise Exception(f"Schema {a1.elementType} and {a2.elementType} cannot be merged")
...
def merge_schemas(s1, s2):
    ...
    for key in s1_keys & s2_keys:
        ...
        elif is_null(s1[key].dataType) or not is_null(s2[key].dataType):
            new_fields[key] = s2[key].dataType
        elif not is_null(s1[key].dataType) or is_null(s2[key].dataType):
            new_fields[key] = s1[key].dataType
        else:
